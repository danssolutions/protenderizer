...
def test_missing_values_handled(raw_data):
    processed = clean_notice(raw_data)   #clean_notice should be changed to the name of the preprocess method
    assert not processed.isnull().values.any()

def test_empty_input(empty_data):
    processed = clean_notice(empty_data)   #clean_notice should be changed to the name of the preprocess method
